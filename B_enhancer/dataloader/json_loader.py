    ...
    def __getitem__(self, index):
        ...
        for filepath in filepaths:
            if 'tiff' in filepath.lower():
                ...
                imgs.extend([Image.fromarray(img, mode='F') for img in _unfolded])
            else:
                with open(filepath, 'rb') as im_handle: # XXX because of Resource Error
                    _img = Image.open(im_handle).convert('RGB')
                    img = _img.copy()
                imgs.append(img)
        ...
        def _aug_mirrorpad2size(imgs):
            ...
            arr_d_lens = [len(array.shape) for array in arrays]
            pads_list = []
            for arr_d_len in arr_d_lens:
                if arr_d_len == 2:
            ...
            arrays = [np.pad(array, pads, mode='symmetric') for array, pads in zip(arrays, pads_list)]
            imgs = [Image.fromarray(array, img.mode) for array, img in zip(arrays, imgs)]
            for img in imgs:
        ...
        if len(filepaths) == 2 and (filepaths[0].endswith("TIFF")) and (filepaths[1].endswith("JPG")):
            assert len(imgs) == 4
            _folded = []
            for img_idx in range(0, len(imgs) - 1, 3):
                _folded.append(np.stack([np.asarray(imgs[img_idx]), np.asarray(imgs[img_idx+1]), np.asarray(imgs[img_idx+2])], -1))
            imgs = _folded + [(np.asarray(imgs[-1])/255.0)]
        elif 'tiff' in filename.lower():
            _folded = []
